...
import person from "../../public/img/products/women-large.jpg";
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (response.data?.status === "OK" && response.data?.data?.products) {
          ...
          setError(`Failed to fetch men's products for ${activeCategory}`);
        }
      } catch (err) {
        console.error(err);
    ...
    <section className="md:py-16 py-4 md:px-14 px-4">
      <div className="container mx-auto">
        <div className="flex flex-wrap -mx-4">
          ...
          >
            <div className="product-large relative h-[500px] bg-cover bg-center bg-no-repeat cursor-pointer transition-transform duration-300 hover:scale-95">
              <Image
                src={person}
              ...
              <div className="absolute inset-0 flex flex-col justify-center items-center text-white">
                ...
                <Link
                  href="#"
                  className="inline-block bg-white hover:bg-customYellow text-black px-6 py-2 rounded-full hover:text-white transition-colors"
                >
                  Discover More
                </Link>
              </div>
            ...
            <div className="ml-4 md:ml-36 mb-4 justify-self-center">
              <ul className="flex items-center space-x-4">
                {categories.map((category, index) => (
                  <li
                    key={index}
                    className={`cursor-pointer ${category === activeCategory ? "text-customYellow font-bold text-xl" : "font-semibold text-gray-600 text-xl hover:text-customYellow"}`}
